				...
				default: //Search features in the feature table
					switch ($searchType) {
						...
						default: //featureTitle, featureSetTitle, format
							...
							$db_query_regexp = 'SELECT id, feature_title, feature_year, format, format_year, feature_set_title, location, existing_loan FROM feature_view WHERE ' . $searchTargetColumn . ' ~* \'.*' . preg_quote($searchInput) . '.*\' AND NOT SIMILAR TO \'' . $searchInput . '\' ' . $orderBy . ';';
					}
			}
